...
def build_parser():
	...
	parser.add_argument("--subsample_threshold", type=float, default=1e-5,
                     help="Threshold for subsampling frequent words (Mikolov et al., 2013). Set to 0 to disable.")
	return parser
...
def main():
    ...
    total_vocab_words = sum(freq_map[w] for w in word_id_map)
    ...
    for word in word_list:
        ...
        if args.subsample_threshold > 0:
            freq = freq_map[word] / total_vocab_words
    ...
    print(f"Ratio of words discarded due to low frequency: {freq_discard_ctr / total_vocab_words}")
    print(f"Number of words discarded due to subsampling: {subsample_discard_ctr / total_vocab_words}")
